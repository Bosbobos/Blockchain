...
def x_change(bin_str1, bin_str2):
    max_len = max(len(bin_str1), len(bin_str2))
    num1 = int(bin_str1, 2) if bin_str1 else 0
    num2 = int(bin_str2, 2) if bin_str2 else 0
...
# главная функция
def stribog_both(M) -> str:
    """
    :param M: это массив в hex (предполагается)
    :return: строка
    """
    M = hex_bin(M)
    N = "0" * 512
